...
{
    private EDrawingWPFControl edrawing;

    public MainWindow()
    {
        EDrawingComponent.UIOptions =
            EMVEnableFeatures.eMVFullUI
            | EMVEnableFeatures.eMVSmallToolbarButtons
            | EMVEnableFeatures.eMVReadOnly
            | EMVEnableFeatures.eMVSuppressSavePrompt
            | EMVEnableFeatures.eMVSupressMenuBar
            | EMVEnableFeatures.eMVSuppressMarkupFileOpen
            | EMVEnableFeatures.eMVSuppressMarkupOpenMenu;
    ...
    private void Button_Click_5(object sender, RoutedEventArgs e)
    {
        ...
        tootipData.TipID = edrawing.EDrawingHost.CreateToolTip(
            tootipData.TipTitle,
            tootipData.TipText,
            tootipData.ShowAtMousePosition,
            tootipData.X,
            tootipData.Y
        );
    ...
    private void Button_Click_9(object sender, RoutedEventArgs e)
    {
        ...
        edrawing.EDrawingHost.Print5(
            printData.ShowDialog,
            printData.FileNameInPrintQueue,
            printData.Shaded,
            printData.DraftQuality,
            printData.Color,
            printData.PrintType,
            printData.Scale,
            printData.CenterOffsetX,
            printData.CenterOffsetY,
            printData.PrintAll,
            printData.PageFirst,
            printData.PageLast,
            printData.PrintToFileName
        );
    }
    ...
    private void Button_Click_12(object sender, RoutedEventArgs e)
    {
        if (!Path.GetExtension(path: edrawing.EDrawingHost.FileName).ToLower().Contains("asm"))
        {
            System.Windows.MessageBox.Show(
                $"You can only use {nameof(edrawing.EDrawingHost.SelectByRay)} in asm file,Current file:{edrawing.EDrawingHost.FileName}"
            );
            return;
        }
        ...
        edrawing.EDrawingHost.SelectByRay(
            data.PointX,
            data.PointY,
            data.PointZ,
            data.DirectionX,
